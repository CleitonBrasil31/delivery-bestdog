      ...
      <div className="max-w-6xl mx-auto p-4 md:p-8">
        ...
        {modalAberto && (
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
            <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-fade-in max-h-[90vh] flex flex-col">
               <div className={`p-4 flex justify-between items-center text-white ${formPedido.id ? 'bg-blue-600' : 'bg-red-600'}`}>
                 <h2 className="font-bold text-lg flex items-center gap-2">{formPedido.id ? <><Pencil size={20}/> Editar Pedido #{formPedido.id}</> : <><Plus size={20}/> Novo Pedido</>}</h2>
                      ...
                      {(pedido.itens || []).map((item, i) => {
                        ...
                        return (
                          <div key={i} className="flex flex-col justify-between text-sm text-gray-800 py-1 border-b border-amber-200/50 last:border-0">
                            ...
                            {adics.length > 0 && adics.map(ad => (<div key={ad} className="text-xs text-green-700 pl-4 font-bold flex justify-between"><span>+ {ad}</span></div>))}
                          </div>
                        )
                      })}
                      {pedido.taxa_entrega > 0 && (<div className="flex justify-between text-sm text-gray-600 py-1 border-b border-amber-200/50 pt-2"><span className="flex items-center gap-1"><Bike size={14}/> Entrega</span><span>R$ {Number(pedido.taxa_entrega).toFixed(2)}</span></div>)}
                      {pedido.desconto > 0 && (<div className="flex justify-between text-sm text-red-600 py-1 border-b border-amber-200/50 font-bold"><span className="flex items-center gap-1"><Percent size={14}/> Desconto ({pedido.desconto}%)</span><span>- R$ {(Number(pedido.total) / (1 - pedido.desconto/100) * (pedido.desconto/100)).toFixed(2)}</span></div>)}
